...
def build_readme(ws, leagues: list[str], total: int, clear: int, budget: int) -> None:
    ...
    structure = [
        ...
        ("Squad",              "Hradec Kral ové current defensive quality benchmarks"),
    ]
    ...
    defs = [
        ("DIS — Defensive Impact Score",
         ...
         "Weights: Successful def actions/90 x3, Def duels won% x2.5, "
         "Interceptions/90 x2, PAdj Interceptions x2, Aerial won% x1.5, Blocks/90 x1"),
        ("ADI — Aerial Dominance Index",
         "Net aerial wins per 90 = (Aerial duels/90) x (Aerial won% / 100), then ranked 0-100. "
         "Combines volume with success quality."),
        ("PADS — Pressure-Adjusted Defensive Score",
         "Uses Wyscout PAdj metrics to neutralise possession-context bias. "
         "Weights: PAdj Interceptions x2.5, PAdj Sliding tackles x2.5, Blocks/90 x1.5, Def duels/90 x1."),
        ("PDS — Positional Discipline Score",
         "Inverted rank of foul cost = Fouls/90 x0.6 + Yellow/90 x0.3 + Red/90 x0.1. "
         "High PDS = defends cleanly, low foul rate, rarely booked."),
        ("JDR — JamesTown Defensive Rating",
         "Master 0-100 composite: DIS x40% + PADS x25% + ADI x20% + PDS x15%, re-percentiled within position."),
...
def build_top5(ws, master: pd.DataFrame) -> None:
    ...
    for pos, label in pos_labels.items():
        ...
        ws.append(["#", "Player", "Team", "League", "Age", "Contract",
                   "Mkt Val (EUR)", "JDR", "DVI", "DIS", "ADI", "PADS", "PDS", "Status"])
        hdr = ws.max_row
        for cell in ws[hdr]:
            cell.font = Font(bold=True, color=C["white"], size=9)
            cell.fill = _fill(C["header"])
...
def build_squad(ws) -> None:
    ...
    ws.append(["FC HRADEC KRALOVE — CURRENT DEFENSIVE QUALITY BENCHMARKS"])
...
def run(leagues: list[str] | None, min_minutes: int, max_age: int,
    ...
    for pos, label in [("GK", "GOALKEEPER TARGETS"), ("CB", "CENTRE-BACK TARGETS"),
                       ("FB", "FULL-BACK TARGETS"), ("DM", "DEFENSIVE MID TARGETS")]:
